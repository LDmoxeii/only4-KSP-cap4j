...
public class ArticleTagSchema {
    ...
    public Schema.Field<Long> tagId() {
        return root == null ? new Schema.Field<>("tagId") : new Schema.Field<>(root.get("tagId"));
    }
    ...
    public Schema.Field<String> tagName() {
        return root == null ? new Schema.Field<>("tagName") : new Schema.Field<>(root.get("tagName"));
    }
    ...
    public static Specification<ArticleTag> specify(Schema.PredicateBuilder<ArticleTagSchema> builder, boolean distinct) {
        return (root, criteriaQuery, criteriaBuilder) -> {
            ...
            criteriaQuery.where(builder.build(articleTag));
     ...
     * @return
     */
    public static Sort orderBy(Schema.OrderBuilder<ArticleTagSchema>... builders) {
     ...
     * @return
     */
    public static Sort orderBy(Collection<Schema.OrderBuilder<ArticleTagSchema>> builders) {
        if(null == builders || builders.isEmpty()) {
            return Sort.unsorted();
        }
        return Sort.by(builders.stream()
                .map(builder -> builder.build(new ArticleTagSchema(null, null)))
