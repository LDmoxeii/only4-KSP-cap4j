...
import com.only4.domain.aggregates.member.ViewHistory;
...
public class ViewHistorySchema {
    ...
    public static Specification<ViewHistory> specify(Schema.PredicateBuilder<ViewHistorySchema> builder, boolean distinct) {
        return (root, criteriaQuery, criteriaBuilder) -> {
            ...
            criteriaQuery.where(builder.build(viewHistory));
     ...
     * @return
     */
    public static Sort orderBy(Schema.OrderBuilder<ViewHistorySchema>... builders) {
     ...
     * @return
     */
    public static Sort orderBy(Collection<Schema.OrderBuilder<ViewHistorySchema>> builders) {
        if(null == builders || builders.isEmpty()) {
            return Sort.unsorted();
        }
        return Sort.by(builders.stream()
                .map(builder -> builder.build(new ViewHistorySchema(null, null)))
